  ...
  try {
    ...
    if (image.user_id && image.user_id !== userId) {
      ...
      if (!users.length || users[0].role !== '管理员') {
        return res.status(403).json({ message: '无权编辑此图片' })
      }
    }
    ...
    if (imageBase64) {
      ...
      try {
        ...
        console.warn('删除旧图片文件失败:', e.message)
      }
      // 保存新文件
      const { url, path: filePath } = saveImageFile(imageBase64)
      updates.push('url = ?'); params.push(url)
      updates.push('path = ?'); params.push(filePath)
    }
    ...
    params.push(id)
    await queryAsync(`UPDATE images SET ${updates.join(', ')} WHERE id = ?`, params)

    const updated = await queryAsync('SELECT * FROM images WHERE id = ?', [id])
